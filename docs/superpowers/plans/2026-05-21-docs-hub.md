...
# machine-specific — copy real values from /root/CLAUDE.md
CF_ZONE_ID=
ORIGIN_IP=
```
...
`STORE_ROOT=/root/docs-hub/store`, the two generated secrets above, and
`CF_ZONE_ID` / `ORIGIN_IP` (machine-specific — copy from `/root/CLAUDE.md`).
...
The zone ID and origin IP are machine-specific (canonical source:
`/root/CLAUDE.md`) and are read from the git-ignored `.env`
(`CF_ZONE_ID`, `ORIGIN_IP`) — they are never committed.

```bash
set -a; . /root/docs-hub/.env; set +a
curl -s -X POST \
  "https://api.cloudflare.com/client/v4/zones/$CF_ZONE_ID/dns_records" \
  -H "Authorization: Bearer $CLOUDFLARE_API_TOKEN" \
  -H "Content-Type: application/json" \
  --data "{\"type\":\"A\",\"name\":\"docs\",\"content\":\"$ORIGIN_IP\",\"proxied\":true}"
```
